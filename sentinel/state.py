...
# Credential combo tracking from PAM capture (user + password pairs)
ssh_combos = Counter()                      # "user||password" -> total attempts across all IPs
ssh_ip_combos = defaultdict(Counter)        # ip -> "user||password" -> attempt count
